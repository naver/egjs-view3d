...
*/
import { Vector2, PCFSoftShadowMap, WebGLRenderer, sRGBEncoding, Clock, Vector3, Scene as Scene$1, Group, PerspectiveCamera, AnimationMixer, Box3, Matrix4, Object3D, Vector4, DirectionalLight, Quaternion, Euler, PlaneGeometry, ShadowMaterial, Mesh, LoadingManager, FileLoader, LoaderUtils, Color, AmbientLight, TextureLoader as TextureLoader$1, CubeTextureLoader, WebGLCubeRenderTarget, RingGeometry, MeshBasicMaterial, DoubleSide, BufferGeometry, LineBasicMaterial, Line, Ray, Plane, CanvasTexture, CircleGeometry, Geometry, Sphere, CylinderBufferGeometry, Texture } from 'three';
...
function getBoxPoints(box) {
  return [box.min.clone(), new Vector3(box.min.x, box.min.y, box.min.z), new Vector3(box.min.x, box.max.y, box.min.z), new Vector3(box.min.x, box.max.y, box.max.z), new Vector3(box.max.x, box.min.y, box.min.z), new Vector3(box.max.x, box.min.y, box.max.z), new Vector3(box.max.x, box.max.y, box.min.z), box.max.clone()];
}
...
function () {
  ...
  __proto.activate = function (_a, gesture) {
    ...
    if (gesture === GESTURE.TWO_FINGER_HORIZONTAL) {
      rotationIndicator.updateRoation(model.scene.quaternion.clone().multiply(new Quaternion().setFromEuler(new Euler(-Math.PI / 2, 0, 0))));
      this._state = STATE$2.ROTATE_HORIZONTAL;
    } else if (gesture === GESTURE.TWO_FINGER_VERTICAL) {
      rotationIndicator.updateRoation(model.scene.quaternion.clone().multiply(new Quaternion().setFromEuler(new Euler(0, Math.PI / 2, 0))));
